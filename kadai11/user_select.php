...
//２．データ登録SQL作成
$stmt = $pdo->prepare("SELECT * FROM kadai10");
$status = $stmt->execute();
      ...
      <?php foreach($values as $v){ ?>
        <tr>
          ...
          <?php }else{ ?>
            <td>一般</td>
          <?php } ?>
          <td><a href="user_detail.php?id=<?=$v['id']?>" class="btn-action btn-update">更新</a></td>
          <td><a href="user_delete.php?id=<?=$v['id']?>" class="btn-action btn-delete">削除</a></td>
        </tr>
      <?php }?>
    </tbody>
  </table>
